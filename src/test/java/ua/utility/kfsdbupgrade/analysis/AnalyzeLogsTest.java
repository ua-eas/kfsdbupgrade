...
public class AnalyzeLogsTest {
    ...
    private ImmutableTable<String, DbUpgradeKey, DbUpgradeSummary> getSummary(List<DbUpgrade> upgrades) {
        // Sorts by chain names, which will have precedence master->develop->onbranch
        ...
        for (DbUpgrade upgrade : upgrades) {
            ...
            TimedInterval timing;
            if (first.getStart() > last.getStop()) {
                // Jenkins had three runs where start was _after_ start; this hack prevents tanking
                // the analyisis run due to this (no idea how jenkins hiccuped like that)
                System.out.printf("Found bad start/stop!!!!\n");
                System.out.printf("\tView: %s\n", upgrade.getView());
                System.out.printf("\tUpgrade run: %s\n", upgrade.getSequence());
                timing = TimedInterval.builder().withStart(first.getStart()).withElapsed(1).withStop(first.getStart() + 1L).build();
            } else {
                timing = TimedInterval.builder().withStart(first.getStart()).withElapsed(elapsed).withStop(last.getStop()).build();
            }
    ...
    private List<DbUpgrade> getDatabaseUpgrades(Iterable<View> views, Table<String, BuildKey, JobResult> table) {
        List<DbUpgrade> list = new ArrayList<>();
        for (View view : views) {
    ...
    private ImmutableList<File> getFilesToZip(Iterable<JenkinsJob> jobs) throws IOException {
        List<File> list = newArrayList();
