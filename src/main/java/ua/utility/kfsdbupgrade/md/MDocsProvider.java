...
public final class MDocsProvider implements Provider<Long> {
  ...
  private void warmup(MDocContext ctx, ExecutorService rds, Iterable<Connection> conns, List<String> rowIds) {
    ...
    // Convert the list of RowId objects (each representing a unique file system block) into row id strings Oracle can understand
    ...
    // Since CLOB data is stored elsewhere on the file system, we'll now use high speed selects to touch 10% of the CLOBS from this table
  ...
  private void doErrors(Iterable<ChunkResult> chunks) {
    List<String> errors = newArrayList();
    for (ChunkResult chunk : chunks) {
      errors.addAll(chunk.getErrors());
    }
    String show = errors.size() > 0 ? Joiner.on(',').join(errors) : "";
    info(LOGGER, "maintenance document conversion errors -> %s %s", getCount(errors.size()), show);
  }
  ...
  private String analyze(ChunkResult chunk, Stopwatch current) {
    ...
    String c = chunk.getConvert().getMillis() > 1 ? " c" + throughput(chunk.getConvert()) + " " : " ";
  ...
  private ChunkResult doChunk(ExecutorService rds, ExecutorService ec2, List<Connection> conns, List<String> rowIds, MDocContext ctx) {
    ...
    List<String> errors = copyOf(concat(read.getErrors(), convert.getErrors(), write.getErrors()));
    return new ChunkResult(rowIds.size(), read.getMetric(), convert.getMetric(), write.getMetric(), errors);
  }
  ...
  private MDocResult convert(ExecutorService ec2, List<MaintDoc> docs, Function<MaintDoc, MaintDoc> function) {
    ...
    List<MaintDoc> success = filter(converted, not(MDocErrorPredicate.INSTANCE));
    List<MaintDoc> errors = filter(converted, MDocErrorPredicate.INSTANCE);
    ...
    return new MDocResult(metric, success, transform(errors, MDocHeaderIdFunction.INSTANCE));
  }
