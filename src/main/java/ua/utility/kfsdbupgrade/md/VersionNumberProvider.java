...
public final class VersionNumberProvider implements Provider<ImmutableList<Integer>> {

  private static final Logger LOGGER = Logger.getLogger(VersionNumberProvider.class);
  ...
  @Override
  public ImmutableList<Integer> get() {
    ...
    try {
      ...
      rs = stmt.executeQuery(format("SELECT VER_NBR FROM %s", from));
      while (rs.next()) {
  ...
  private VersionNumberProvider(Builder builder) {
  ...
  public static VersionNumberProvider build(Connection conn, String table, int max, int show) {
  ...
  public static class Builder {
    ...
    public VersionNumberProvider build() {
      return validate(new VersionNumberProvider(this));
    }

    private static VersionNumberProvider validate(VersionNumberProvider instance) {
