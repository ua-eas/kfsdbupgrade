...
public final class RowsProvider<T> implements Provider<ImmutableList<T>> {
  ...
  public ImmutableList<T> get() {
    try {
      ...
      int processed = 0;
      for (List<String> partition : partition(rowIds, chunkSize)) {
        Stopwatch current = createStarted();
        int index = 0;
        List<Callable<ImmutableList<T>>> callables = newArrayList();
        for (List<String> distribution : distribute(partition, conns.size())) {
        ...
        progress(processed, overall, partition.size(), current);
      }
      return copyOf(list);
    } catch (Throwable e) {
  ...
  private void progress(int processed, Stopwatch overall, int partitionSize, Stopwatch current) {
    String all = getThroughputInSeconds(overall.elapsed(MILLISECONDS), processed, "rows/sec");
    String now = getThroughputInSeconds(current.elapsed(MILLISECONDS), partitionSize, "rows/sec");
