...
public class MaintainableXMLConversionServiceimpl2 implements MaintainableXmlConversionService {
  private static final Logger LOGGER = Logger.getLogger(MaintainableXMLConversionServiceimpl2.class);
  ...
  /**
   * Populated by the <code>pattern</code> elements in the <code>rule</code> named <code>maint_doc_classname_changes</code> in {@link #rulesXmlFile}. See the {@link #rulesXmlFile}
   * for more detail.
   */
  private Map<String, String> classNameRuleMap;
  /**
   * Populated by the <code>pattern</code> elements in the <code>rule</code> named <code>maint_doc_changed_class_properties</code> in {@link #rulesXmlFile}.
   * {@link #setupConfigurationMaps()} also pre-populates with some values that are applicable to all BOs. See the {@link #rulesXmlFile} for more detail.
   */
  private Map<String, Map<String, String>> xPathTransformationRulesMap;

  /**
   * Populated by the <code>pattern</code> elements in the <code>rule</code> named <code>xpath_specific_changes</code> in {@link #rulesXmlFile}. {@link #setupConfigurationMaps()} ]
   */
  private Map<String, Map<String, String>> classPropertyRuleMap;

  /**
   * Populated by the <code>pattern</code> elements in the <code>rule</code> named <code>maint_doc_date_changes</code> in {@link #rulesXmlFile}. See the {@link #rulesXmlFile} for
   * more detail.
   */
  private Map<String, String> dateRuleMap;
  /**
   * {@link File} containing the rule maps that will be used to transform the maintainable document XML.
   */
  private final File rulesXmlFile;
  /**
   * {@link Set} of {@link String}s representing classnames to ignore during transformation. Values are hardcoded and passed in during construction.
   */
  private Set<String> ignoreClassSet = new HashSet<String>();
  /**
   * If a {@link String} classname that begins with <code>edu.arizona</code> or <code>com.rsmart</code> is encountered in {@link #isValidClass(Class)} , that classname will be
   * added to this {@link Set}. If it's the first time such a class has been encountered, it will be logged that that class is being skipped by processing.
   */
   ...
   * @throws Exception
   */
  public MaintainableXMLConversionServiceimpl2(File rulesXmlFile) throws Exception {
  ...
  public MaintainableXMLConversionServiceimpl2(File rulesXmlFile, Level logLevel) throws Exception {
    this(rulesXmlFile);
    LOGGER.setLevel(logLevel);
  }

  /**
   * Transforms the given <code>xml</code> that is in KFS3 format to KFS6 format.
   * 
   * @param xml
  ...
  public String transformMaintainableXML(String xml) throws Exception {
    ...
    /*
     * a handful of documents have unfriendly Unicode characters which the XML processor (and the rest of KFS) can't handle. Pre-process to replace with a friendly base ASCII
     * characters.
     */
    xml = xml.replace("\u0001", "-");
    xml = xml.replace("\u001e", " ");
    ...
    for (String className : classNameRuleMap.keySet()) {
      if (beginning.contains("maintainableImplClass=\"" + className + "\"")) {
        LOGGER.trace("Replacing top-level maintainableImplClass attribute: " + className + " with: " + classNameRuleMap.get(className));
    ...
    String convertedXML = beginning + "<" + OLD_MAINTAINABLE_OBJECT_ELEMENT_NAME + ">" + convertedOldMaintainableObjectXML + "</" + OLD_MAINTAINABLE_OBJECT_ELEMENT_NAME + ">" + "<"
        + NEW_MAINTAINABLE_OBJECT_ELEMENT_NAME + ">" + convertedNewMaintainableObjectXML + "</" + NEW_MAINTAINABLE_OBJECT_ELEMENT_NAME + ">" + ending;
    return convertedXML;
  }

  /**
   * Transforms the given <code>xml</code> section from KFS3 format to KFS6 format.
   * 
   * @param xml
   *          {@link String} of the XML to transform
   * @return {@link String} of the transformed XML
  ...
  private String transformSection(String xml) throws Exception {
    ...
    try {
      ...
      String exMsg = "Failed in db.parse(new InputSource(new StringReader(xml))) where xml=" + xml + eol + "of maintenanceAction = " + maintenanceAction + eol
    ...
    /*
     * the default logic that traverses over the document tree doesn't handle classes that are in an @class attribute, so we deal with those individually.
     */
    ...
    /*
     * (?m) puts the regex into multiline mode: https://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern. html#MULTILINE So the effect of this statement is
     * "remove any empty lines"
     */
    xml = writer.toString().replaceAll("(?m)^\\s+\\n", "");
    xml = xml + "<" + MAINTENANCE_ACTION_ELEMENT_NAME + ">" + maintenanceAction + "</" + MAINTENANCE_ACTION_ELEMENT_NAME + ">";

    // replace classnames not updated so far that were captured by smoke test below
    for (String className : classNameRuleMap.keySet()) {
      if (xml.contains("active defined-in=\"" + className + "\"")) {
        LOGGER.info("Replacing active defined-in= attribute: " + className + " with: " + classNameRuleMap.get(className));
        xml = xml.replace("active defined-in=\"" + className + "\"", "active defined-in=\"" + classNameRuleMap.get(className) + "\"");
      }
    }
  ...
  /*
   * works more predictably and completely than existing traversal, however eats up much more time and resources (takes 6x longer to run, and pegs the toolsbox cpu the whole time)
   */
  @SuppressWarnings("unused")
  private void doXpathTransformations(Document document) {
    /*
     * note that order matters; if we map classnames first, class properties' XPaths will fail
     */
    mapClassPropertiesWithXPath(document);
    mapClassNamesWithXPath(document);
    /*
     * leaving date transformation in original traversal, and also leaving original as there is additional transformation business logic in that code. So some duplication, but much
     * correctness
     */
  }

  /**
   * Map the classnames as described in {@link #classNameRuleMap}, finding the elements to transform via XPath against the top level {@link Document}.
   * 
   * @param document
   */
  ...
  /**
   * Map the class properties as described in {@link #classPropertyRuleMap}, finding the elements to transform via XPath against the top level {@Link Document}.
   * 
   * @param document
   */
        ...
        for (int i = 0; i < matchingNodes.getLength(); i++) {
          ...
          for (String propertyName : propertyMappings.keySet()) {
            ...
            for (int j = 0; j < propertyNodes.getLength(); j++) {
              ...
              /*
               * if map target is empty, remove the node. Otherwise, rename to value.
               */
  ...
  /**
   * There is an edge case in the main traversal such that a TypedArrayList element that is a child of a non-TypedArrayList element will be missed by processing. The traversal
   * logic is tangled to the point that (correctly) fixing at that level will likely introduce more bugs, so doing cleanup after the fact instead.
   * 
   * @param document
   ...
   */
  private void catchMissedTypedArrayListElements(Document document) throws Exception {
    ...
    for (String className : classNameRuleMap.keySet()) {
      try {
        ...
        for (int i = 0; i < matchingNodes.getLength(); i++) {
          ...
          LOGGER.trace("In element " + matchingNodes.item(i).getNodeName() + " migrating @class attribute of " + className + " to " + targetClassName);
        }
      } catch (XPathExpressionException e) {
        LOGGER.error("XPathException encountered: ", e);
      }
  ...
  /**
   * Migrate @maintainableImplClass attributes which are missed by the main iteration
   * 
   * @param document
   */
  /*
   * this has to be hacked around at the top-level before the DOM is even created; leaving code in place to hopefully someday do this at the DOM level
   */
  @SuppressWarnings("unused")
    ...
    for (String className : classNameRuleMap.keySet()) {
      try {
        ...
        for (int i = 0; i < matchingNodes.getLength(); i++) {
          Node classAttr = matchingNodes.item(i).getAttributes().getNamedItem("class");
          classAttr.setNodeValue(targetClassName);
          LOGGER.info("In element " + matchingNodes.item(i).getNodeName() + " migrating @maintainableImplClass attribute of " + className + " to " + targetClassName);
        }
      } catch (XPathExpressionException e) {
        LOGGER.error("XPathException encountered: ", e);
      }
  ...
  /*
   * main traversal not finding and removing all autoIncrementSet elements, so cleaning up after it. TODO remove this method once main traversal is fixed
   */
  ...
  @SuppressWarnings("unused")
  /*
   * used when debugging, might need to re-enable, not throwing away code just yet
   */
  private void removeAssetExtensionElements(Document document) {
    ...
    try {
      ...
      for (int i = 0; i < matchingNodes.getLength(); i++) {
        Node match = matchingNodes.item(i);
        Node parent = match.getParentNode();
        LOGGER.info("Removing element 'edu.arizona.kfs.module.cam.businessobject.AssetExtension' in " + parent.getNodeName());
        parent.removeChild(match);
      }
    } catch (XPathExpressionException e) {
      LOGGER.error("XPathException encountered: ", e);
  ...
  /**
   * Investigative logging. Log if there are any elements with an \@class attribute
   * 
   * @param document
   */
  ...
  private String upgradeBONotes(String oldXML) throws Exception {
    ...
    if (notesXml != null) {
      ...
      if ((pos1 > -1) && (pos2 > pos1)) {
        oldXML = (oldXML.substring(0, pos1) + ">\n<boNotes>\n" + notesXml + "\n</boNotes>" + oldXML.substring(pos2 + "</boNotes>".length()));
      }
    }
  ...
  /**
   * Migrate any elements with the <code>class</code> containing <code>PersonImpl</code> from the provided {@link Document} if there is a mapping in {@link #classNameRuleMap}.
   * 
   * @param doc
   */
  public void migratePersonObjects(Document doc) {
    ...
    try {
      ...
      for (int i = 0; i < matchingNodes.getLength(); i++) {
        ...
        String line1 = null, line2 = null, line3 = null, city = null, stateProvinceCode = null, postalCode = null, countryCode = null;
        ...
        if (!(line1 == null || line1.isEmpty()) || !(line2 == null || line2.isEmpty()) || !(line3 == null || line3.isEmpty()) || !(city == null || city.isEmpty())
            || !(stateProvinceCode == null || stateProvinceCode.isEmpty()) || !(postalCode == null || postalCode.isEmpty()) || !(countryCode == null || countryCode.isEmpty())) {
  ...
  /**
   * Migrate any elements with the <code>class</code> containing <code>KualiCodeBase</code> from the provided {@link Document} if there is a mapping in {@link #classNameRuleMap}.
   * 
   * @param doc
   */
  ...
  public void migrateAccountExtensionObjects(Document doc) {
    ...
    try {
      accountExtensionProperties = xpath.compile("//*[@class='edu.arizona.kfs.coa.businessobject.AccountExtension']");
      NodeList matchingNodes = (NodeList) accountExtensionProperties.evaluate(doc, XPathConstants.NODESET);
      for (int i = 0; i < matchingNodes.getLength(); i++) {
        Node tempNode = matchingNodes.item(i);
        LOGGER.trace("Migrating AccountExtension node: " + tempNode.getNodeName() + "/" + tempNode.getNodeValue());
  ...
  /**
   * If a mapping for the <code>node</code> class exists in {@link #classNameRuleMap}, the given <code>node</code> is renamed. Then, if the <code>node</code> is a valid class, it
   * is passed to the {@link #transformNode(Document, Node, Class, Map)} method first with the {@link #classPropertyRuleMap} value for the classname, then with the
   ...
   * @throws InstantiationException
   */
  private void transformClassNode(Document document, Node node) throws Exception {
   ...
   * Does the following:
   * <ol>
   * <li>Recursively calls this method on all child elements of <code>talist</code> to handle any child lists first
   * <li>Remove the attributes {@link #SERIALIZATION_ATTRIBUTE} and {@link #CLASS_ATTRIBUTE} of <code>talist</code></li>
   * <li>If <code>//[talist.getNodeName()]/org.apache.ojb.broker.core.proxy.ListProxyDefaultImpl/default/size/</code> evaluates to a value >1, indicating elements in this list,
   * call {@link #transformClassNode(Document, Node)} on that element and store to readd</li>
   * <li>Remove all child elements of <code>talist</code></li>
   * <li>Read list elements calculated and transformed above</li>
   * </ol>
   * 
   * @param document
   *          Root {@link Document}
   ...
   * @throws InstantiationException
   */
  private void handleTypedArrayList(Document document, XPath xpath, Element talist) throws Exception {
    LOGGER.trace("Handling typed array list: " + talist.getNodeName());
    XPathExpression getChildTypedArrayLists = xpath.compile(".//*[@class='org.kuali.rice.kns.util.TypedArrayList']");
  ...
  private void transformNode(Document document, Node node, Class<?> currentClass, Map<String, String> propertyMappings) throws Exception {
    LOGGER.trace("Transforming node: " + node.getBaseURI() + "/" + node.getNodeName());
    for (Node childNode = node.getFirstChild(); childNode != null;) {
      Node nextChild = childNode.getNextSibling();
      String propertyName = childNode.getNodeName();
      if (childNode.hasAttributes()) {
        doAttributes(childNode, propertyName, document);
      }
      ...
      if ((currentClass != null) && isValidClass(currentClass)) {
        if (childNode.hasChildNodes() && !(Collection.class.isAssignableFrom(currentClass) || Map.class.isAssignableFrom(currentClass))) {
          ...
          Optional<Class<?>> propertyClass = propertyClassCache.get(key);
  ...
  private void doAttributes(Node childNode, String propertyName, Document document) throws Exception {
    NamedNodeMap nnm = childNode.getAttributes();
    Node serializationAttribute = nnm.getNamedItem(SERIALIZATION_ATTRIBUTE);
    if (serializationAttribute != null && StringUtils.equals(serializationAttribute.getNodeValue(), "custom")) {
      XPath xpath = XPathFactory.newInstance().newXPath();
      Node classAttribute = nnm.getNamedItem(CLASS_ATTRIBUTE);
  ...
  /**
   * Storefront to call {@link #isValidClass(String)} with {@link Class#getName()}
   * 
   * @param c
   * @return value of {@link #isValidClass(String)} with {@link Class#getName()}
   */
   ...
   * @return <code>true</code> if <code>className</code> does NOT start with <code>edu.arizona</code> or <code>com.rsmart.</code>, and <code>className</code> is not in the
   *         {@link #ignoreClassSet}.
   */
  ...
  /**
   * Constructs the various instance variable maps and pre-populates {@link #classPropertyRuleMap} with some values which apply to all BOs.
   */
